    ...
    <main class="flex-1 overflow-y-auto p-6">
      ...
      <section class="mb-6">
        ...
        <div class="mb-4">
          ...
          <select id="categoryFilter" v-model="selectedCategory" @change="filterCourses" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
      ...
      <section class="mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <button
              @click="enrollInCourse(course.title)"
...
export default {
  ...
  mounted() {
    ...
    if (userStr) {
      ...
      this.fetchEnrolledCourses(user.email);
    } else {
      this.studentName = "Student";
    }
  },
  methods: {
    async fetchEnrolledCourses(studentEmail) {
      try {
        const response = await fetch(`http://localhost:8080/api/enroll/courses/${encodeURIComponent(studentEmail)}`);
        if (response.ok) {
          ...
          // Additional logic to calculate completedCourses, hoursLearned, overallCompletion can be added here
        } else {
    ...
    filterCourses() {
      // This method can be used to filter courses based on selectedCategory
      // For now, since the backend doesn't support filtering by category in the endpoint,
      // we can filter the already fetched courses
      if (this.selectedCategory) {
        this.enrolledCourses = this.enrolledCourses.filter(course => course.category === this.selectedCategory);
      } else {
        // If no category selected, refetch all courses
        const userStr = localStorage.getItem("user");
        if (userStr) {
          const user = JSON.parse(userStr);
          this.fetchEnrolledCourses(user.email);
